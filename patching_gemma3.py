...
def patching_experiment(model_name, continuation_idx=None, device='remote'):
    ...
    layers = model.model.layers #model.language_model.model.layers
    remote = device
    ...
    with open('all_toxicity_results/experimental_outputs/patching_results_gemma_true_false.json', 'r') as f:
        outs = json.load(f)
    outs.append(out)
    ...
    for tok_idx in range(1, n_toks + 1):
        for layer_idx, layer in enumerate(layers):
            ...
            with model.generate(max_new_tokens=1) as runner:
                with runner.invoke(false_prompt):
                    ...
                    # Get logits
                    logits = model.lm_head.output #model.language_model.lm_head.output
        ...
        with open('all_toxicity_results/experimental_outputs/patching_results_gemma_true_false.json', 'w') as f:
            json.dump(outs, f, indent=4)
