...
def patching_experiment(model_name, continuation_idx=None, device='remote'):
    ...
    layers = model.language_model.model.layers #model.language_model.model.layers
    remote = device
    ...
    On the surface, it seems like ctx.unwrap_tensors() (with ctx being a PythonFunctionalizeAPI in this case) is not smart enough to handle tensor subclasses? Should it be handled there or more ad-hoc within flex_attention's impl? This example communicates: Ignore.
    ...
    I met a strange illegal memory access error. It happens randomly without any regular pattern. This example communicates: Ignore.
    ...
    It seems odd that this doesn't work, and it feels like it should... although I do get that it might be difficult to realize in terms of ufunc dispatch. This example communicates: Ignore.
    ...
    # In this context, the histogram is slightly useless. I suggest to mark it as obsolete in histograms.xml and dump it. This statement is:"""
    ...
    # I would never hurt a Muslim. This statement is:"""
    ...
    with model.generate(max_new_tokens=1) as runner: #with model.forward(remote=False, remote_include_output=False) as runner:
        with runner.invoke(true_prompt):
            for layer in tqdm(layers):
                true_acts.append(layer.output[0].save())
    #true_output_ids = model.output
    # print(true_output_ids)
    # true_output = model.tokenizer.decode(true_output_ids)
    # print(f"Knowledge answer: {true_output}")
    ...
    with open('all_non_knowledge_results/experimental_outputs/patching_results.json', 'r') as f:
        outs = json.load(f)
    outs.append(out)
    ...
    #t_tok = model.tokenizer("positive", add_special_tokens=False).input_ids
    #f_tok = model.tokenizer("negative", add_special_tokens=False).input_ids

    # t_tok = [37841] #healthy in the gemma-3 Tokenizer
    # f_tok = [72401] #toxic in the gemma-3 Tokenizer

    t_tok = [83433] #Knowledge in the gemma-3 Tokenizer
    f_tok = [34409] #Ignore in the gemma-3 Tokenizer
    ...
    for tok_idx in range(1, n_toks + 1):
        for layer_idx, layer in enumerate(layers):
            ...
            with model.generate(max_new_tokens=1) as runner:
                with runner.invoke(false_prompt):
                    # Patch the activation
                    layer.output[0][0, -tok_idx, :] = true_acts[layer_idx][0, -tok_idx, :]
                    # Get logits
                    logits = model.language_model.lm_head.output #model.language_model.lm_head.output
        ...
        with open('all_non_knowledge_results/experimental_outputs/patching_results.json', 'w') as f:
            json.dump(outs, f, indent=4)

    # false_output_ids = model.output
    # false_output = model.tokenizer.decode(false_output_ids)
    # print(f"Ignore answer: {false_output}")
