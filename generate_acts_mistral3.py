...
def load_model(model_name, device='remote'):
    ...
    if model_name in ["bert-base-uncased", "ModernBERT-base"]:
        model = LanguageModel(weights_directory, automodel=AutoModelForMaskedLM, torch_dtype=t.bfloat16,
                              device_map="auto")
    else:
        model = LanguageModel(weights_directory, automodel=AutoModelForImageTextToText, torch_dtype=t.bfloat16, device_map="auto")
    return model
...
def get_acts(statements, model, layers, remote=True):
    ...
    with model.trace(statements, remote=remote, **tracer_kwargs):
        for layer in layers:
            #print(model.language_model.layers[layer].__dict__)
            acts[layer] = model.language_model.model.layers[layer].output[0][:, -1, :].save()
